...
class MCNode(Node):
    ...
    def sample(self,g):
       ...
       |-------------------
        ...
        a_rand_player = RandomPlayer()
        e = g.run_a_game(a_rand_player, a_rand_player, s=self.s)
    ...
    def expand(self,g):
       ...
       |-------------------------------
        ...
        for i in range(len(m)):
            ...
            self.c.append(MCNode(m[i][1], self, [], (m[i][0])))

        c = self.c[np.random.randint(0, len(self.c))]
        #########################################
        return c
    ...
    def compute_UCB(vi, ni, N, x ,c=1.142):
        ...
        if ni == 0:
            return float('inf')
        exploit = x * vi/ni
        b = exploit + c * (np.sqrt(np.log(N)/ni))
        #########################################
        return b
    ...
    def select_a_child(self):
        ...
        curr_scores = []
        curr_highest = (-99999, 0)

        for i in range(len(self.c)):
            # curr_scores.append(self.compute_UCB(self.c[i].v, self.c[i].N, self.N, self.s.x))
            score = self.compute_UCB(self.c[i].v, self.c[i].N, self.N, self.s.x)
            if score > curr_highest[0]:
                curr_highest = score, i
            elif score == curr_highest[0]:
                if self.c[i].N < self.c[curr_highest[1]].N:
                    curr_highest = score, i

        c = self.c[curr_highest[1]]
...
class MCTSPlayer(Player):
    ...
    def choose_optimal_move(self,n):
        ...
        value_index = []
        for c in n.c:
            value_index.append(c.N)

        best = n.c[value_index.index(max(value_index))]
        r,c = best.m
