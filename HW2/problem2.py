...
def import_team_names(filename ='ncaa_teams.txt'):
    ...
    team_names = np.loadtxt(filename, str, '#', '/n').tolist()
...
def import_W(filename ='ncaa_results.csv'):
    ...
    W = np.loadtxt(filename, int, delimiter=',')
...
def team_rating(resultfile = 'ncaa_results.csv', 
    ...
    R = elo_rating(game_results, len(team_names), K)

    # sort team names according to their Elo ratings

    sorted_combined = sorted(zip(R, team_names))[::-1]

    top_ratings = [y for y, x in sorted_combined]
    top_teams = [x for y, x in sorted_combined]
