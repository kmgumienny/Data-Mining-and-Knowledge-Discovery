...
def rank_BA(D, min_AB=300,max_salary=1200000):
    ...
    R = D.sort_values(by=['BA'], ascending=False)['playerID'].tolist()
...
def rank_OBP(D,min_AB=300,max_salary=1200000):
    ...
    D.loc[D['salary'] > max_salary, 'OBP'] = 0
    D.loc[D['AB'] < min_AB, 'OBP'] = 0

    R = D.sort_values(by=['OBP'], ascending=False)['playerID'].tolist()
    #########################################
    return R 
