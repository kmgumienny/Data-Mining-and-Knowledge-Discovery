...
def find_e2(L,tol= 1e-4):
    ...
    vector, values = np.linalg.eig(L)
    vector = vector.tolist()
    vect_sort = sorted(vector, key=lambda x: x.real)

    for i in range(len(vect_sort)):
        val = vect_sort[i].real
        if 0 < val and val > tol:
            index = val
            break

    e2 = values[:, vector.index(index)]
